...
test_input = ['What is the hometown of the reigning mens U.S. Open champion',
              'n',
              'n',
...
def construct_index(directory_path,test_input):
    max_input_size = 4096
    num_outputs= 256
...
def promptf(question, prompt, q_index,test_input,intermediate="\nIntermediate answer:", followup="Follow up:",
    ...
    while followup in get_last_line(ret_text):
        ...
        get_answer(question)
        external_answer = test_input[count+1+3*q_index]
...
def ask_bot(input_index = 'index.json',test_input=test_input):
  index = GPTSimpleVectorIndex.load_from_disk(input_index)
  total_q = 0
  while True and total_q<10:
    query = test_input[0+3*total_q]#input('What do you want to ask the bot?   \n')
    if query == 'quit':
        return 0
    else:
        ...
        query_int = test_input[1+3*total_q]
        if query_int=='y':
            print('======Move to the next question=======')
        elif query_int=='n':

            print('We try to find a intermediate question for you\n')
            res = promptf(query,prompt,q_index=total_q,test_input=test_input)
            if res=='n':
...
if __name__ == "__main__":
    ...
    for test_url in test_urls:
        ...
        index = construct_index(text_path,test_input)
       ...
       # print(time_cost,'//////')
    print('time cost:', time_total/len(test_urls))